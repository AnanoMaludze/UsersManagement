...
namespace UsersManagement.Handlers.Commands
{
    public class CreatePersonCommandValidator : AbstractValidator<CreatePersonCommand>
    {
        public CreatePersonCommandValidator()
        {
            ...
            RuleForEach(x => x.RelatedIndividuals)
                .ChildRules(relatedIndividual => {
                    relatedIndividual.RuleFor(x => x.TypeOfConnection)
                        .IsInEnum().WithMessage("Type of connection must be Colleague, Acquaintance, Relative, or Other.");
                    relatedIndividual.RuleFor(x => x.RelatedPersonId)
                        .NotEmpty().WithMessage("Related person identifier is required.");
                });
        }
